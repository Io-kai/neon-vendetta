...
export class Player extends Fighter {
  ...
  syncWeaponSprite(): void {
    if (this.weapon) {
      if (!this.weaponImg) {
        this.weaponImg = this.scene.add.image(this.fx, this.fy, `item_${this.weapon.type}`);
      ...
      const hx = this.fx + this.facing * (swinging ? 30 : 12);
      const hy = this.fy - this.fz - (swinging ? 40 : 34);
      this.weaponImg.setPosition(hx, hy);
      this.weaponImg.setAngle(this.facing > 0 ? (swinging ? 12 : 70) : (swinging ? 168 : 110));
